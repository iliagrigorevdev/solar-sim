...
void Quadtree::insert(CelestialBody* body) {
    if (body->x < boundary_.x - boundary_.half_dim || body->x > boundary_.x + boundary_.half_dim ||
        body->y < boundary_.y - boundary_.half_dim || body->y > boundary_.y + boundary_.half_dim) {
...
void Quadtree::query(const Boundary& range, std::vector<CelestialBody*>& found) {
    ...
    for (auto& body : bodies_) {
        if (body->x >= range.x - range.half_dim && body->x <= range.x + range.half_dim &&
            body->y >= range.y - range.half_dim && body->y <= range.y + range.half_dim) {
...
void Quadtree::calculate_force(CelestialBody& body, float theta, float G, float softening_factor) {
    ...
    if ((boundary_.half_dim * 2.0f) / dist < theta && bodies_.size() > 1) {
        ...
        float force_magnitude = (G * body.mass * total_mass_) / (dist_sq + softening_factor * softening_factor);
        float dir_x = dx / dist;
        float dir_y = dy / dist;
        ...
        for (auto& other_body : bodies_) {
            ...
            float dist_sq_b = dx_b * dx_b + dy_b * dy_b + softening_factor * softening_factor;
...
const std::vector<std::unique_ptr<Quadtree>>& Quadtree::get_children() const {
    // This is a bit of a hack to return a consistent type, even though the children are stored as individual members.
    // For a real application, you might want to refactor how children are stored if you need to iterate over them frequently.
    static std::vector<std::unique_ptr<Quadtree>> children_vector;
    children_vector.clear();
    if(divided_){
        //This is not ideal as it would involve some sort of transfer of ownership or raw pointers
        //for simplicity we will just leave this empty as it is for debugging
    }
